...
namespace MyGOBot.Logic {
	...
	public class Inventory {
		...
		public async Task<IEnumerable<PokemonData>> GetDuplicatePokemonToTransfer(
			...
			var pokemonToTransfer = myPokemonList.Where(p => pokemonsToTransferArg.Contains(p.PokemonId) && p.DeployedFortId == string.Empty && p.Favorite == 0).ToList();
					...
					p => {
						...
						return (!((p.Cp >= pokemonTransferFilter.KeepMinCp &&
								  PokemonInfo.CalculatePokemonPerfection(p) >= pokemonTransferFilter.KeepMinIvPercentage) ||
										pokemonTransferFilter.Moves.Intersect(new[] { p.Move1, p.Move2 }).Any() == true));
					}).ToList();
		...
		public async Task<IEnumerable<PokemonData>> GetPokemonToEvolve(IEnumerable<PokemonId> filter = null) {
			...
			if (pokemonIds.Any()) {
				myPokemon =
					myPokemon.Where(
						p => (_client.Settings.EvolveAllPokemonWithEnoughCandy && pokemonIds.Contains(p.PokemonId)));
			}
		...
		public TransferFilter GetPokemonTransferFilter(PokemonId pokemon) {
			return new TransferFilter(_client.Settings.KeepMinCP, Convert.ToSingle(_client.Settings.KeepMinIVPercentage), _client.Settings.KeepMinDuplicatePokemon);
		}
